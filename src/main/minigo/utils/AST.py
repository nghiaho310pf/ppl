...
class VarDecl(Decl, BlockMember):
    ...
    def __str__(self):
        return "VarDecl(" + self.varName + (("," + str(self.varType)) if self.varType else "") + (
            "" if self.varInit is None else ("," + str(self.varInit))) + ")"
...
class ConstDecl(Decl, BlockMember):
    ...
    def __str__(self):
        return "ConstDecl(" + self.conName + (("," + str(self.conType)) if self.conType else "") + "," + str(
            self.iniExpr) + ")"
...
class FuncDecl(Decl):
    ...
    def __str__(self):
        return "FuncDecl(" + self.name + ",[" + ','.join(str(i) for i in self.params) + "]," + str(
            self.retType) + "," + str(self.body) + ")"
...
class StructType(Type):
    ...
    def __str__(self):
        return "StructType(" + self.name + ",[" + ','.join(
            ("(" + i + "," + str(j) + ")") for i, j in self.elements) + "],[" + ','.join(
            str(i) for i in self.methods) + "])"
...
class MethCall(Expr, Stmt):
    ...
    def __str__(self):
        return "MethodCall(" + str(self.receiver) + "," + self.metName + ",[" + ','.join(
...
class ArrayLiteral(Literal):
    ...
    def __str__(self):
        return "ArrayLiteral([" + ','.join(str(i) for i in self.dimens) + "]," + str(self.eleType) + "," + nested2Str(
            self.value) + ")"
...
class StructLiteral(Literal):
    ...
    def __str__(self):
        return "StructLiteral(" + self.name + ',[' + ','.join(
            ("(" + str(i) + "," + str(j) + ")") for i, j in self.elements) + "])"
